defmodule Hello.Sys.Company do
 ...
 field :short_name, :string
  ...
  def changeset(company, attrs) do
    company
    |> cast(attrs, [:parent_id, :ancestors, :order_num, :code, :name, :short_name, :area_id, :corp_code, :corp_name, :status,  :remark])
   #|>Ecto.Changeset.put_change( :status,  0)
    ...
    |>validate_length( :ancestors, max: 255,message: "#{dgettext("sys_company","Ancestors")} #{gettext("max length is")} 255")
   ...
   # |>validate_required([:corp_code], message: "#{gettext("Please inpput")}#{dgettext("sys_company","Corp code")}")
    |>validate_length( :corp_code, max: 255,message: "#{dgettext("sys_company","Corp code")} #{gettext("max length is")} 255")

   # |>validate_required([:corp_name], message: "#{gettext("Please inpput")}#{dgettext("sys_company","Corp name")}")
    |>validate_length( :corp_name, max: 255,message: "#{dgettext("sys_company","Corp name")} #{gettext("max length is")} 255")
    ...
    |>validate_length( :short_name, max: 255,message: "#{dgettext("sys_company","Short name")} #{gettext("max length is")} 255")
